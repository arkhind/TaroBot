...
async def process_user_nickname(vox: AsyncVoxAPI, nickname, prompt):
    try:
        ...
        report = await vox.custom_report(subject=Subject.USER, subject_id=user_id,
...
async def process_user_nicknames(vox: AsyncVoxAPI, from_user, about_user, prompt):
    ...
    try:
        ...
        airep_from_user = await vox.ai_analytics(subject=Subject.USER, subject_id=from_user_id)
        ...
        airep_about_user = await vox.ai_analytics(subject=Subject.USER, subject_id=about_user_id)
        if 'report' in airep_about_user:
            airep_about_user = airep_about_user['report']

        report = await vox.custom_report(subject=Subject.USER, subject_id=about_user_id,
