...
from aiogram.types import InlineQuery, InlineQueryResultArticle, InputTextMessageContent, InlineKeyboardMarkup, InlineKeyboardButton, InlineQueryResultUnion, CallbackQuery
...
from prompts import prediction_prompt, qualities_prompt, answers_prompt, yes_no_prompt, compatibility_prompt
...
async def inline_prediction_handler(inline_query: InlineQuery, vox: AsyncVoxAPI):
    ...
    logger.info(f"[INLINE] Получен запрос: '{query}' от user_id={inline_query.from_user.id if inline_query.from_user else 'unknown'}")
        ...
        results = [
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Совместимость двух людей", callback_data=f"get_comp2_{nick1}_{nick2}")]
                    ]
                )
            )
        ]
        logger.info(f"[INLINE] Кнопка совместимости двух людей: {results[0].reply_markup.inline_keyboard if results[0].reply_markup else None}")
        await inline_query.answer(results=results, cache_time=1)
        return

    is_nickname = query.startswith('@') or (query and all(c.isalnum() or c == '_' for c in query) and len(query) < 30)
    ...
    if is_nickname:
        ...
        results = [
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Получить предсказание", callback_data=f"get_pred_{clean_nickname}")]
                    ]
                )
            ),
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Анализировать качества", callback_data=f"get_qual_{clean_nickname}")]
                    ]
                )
            ),
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Совместимость", callback_data=f"get_comp_{clean_nickname}")]
                    ]
                )
            )
        ]
        for r in results:
            if r.reply_markup and hasattr(r.reply_markup, 'inline_keyboard'):
                logger.info(f"[INLINE] Кнопки для никнейма: {r.reply_markup.inline_keyboard}")
            else:
        ...
        results = [
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Получить ответ", callback_data=f"get_q_{query[:50]}")],
                        [InlineKeyboardButton(text="Получить ответ да/нет", callback_data=f"get_yesno_{query[:50]}")]
                    ]
                )
            )
        ]
...
""" + prediction_prompt
        ...
        logger.exception(f"[CALLBACK] Ошибка при получении предсказания для @{nickname}: {e}")
...
async def handle_get_qualities(callback: CallbackQuery, vox: AsyncVoxAPI):
    ...
    try:
        ...
        target_qualities = await process_user_nickname(vox, nickname, qualities_prompt["people_qualities"])
        if target_qualities:
...
async def handle_get_yesno(callback: CallbackQuery, vox: AsyncVoxAPI):
    ...
    try:
        ...
        yesno_prompt_full = f"Вопрос: {question}\n\nДай ответ Да или Нет с подробным объяснением." + yes_no_prompt
